...
1. 可通过 `SHORTS_TEST_ALLOW_ANY_BGM=1` 临时启用“非授权校验”的背景音乐测试。
2. 默认测试音轨为 `input/bgm.mp3`，可用 `SHORTS_TEST_BGM_PATH` 指定其它路径。
3. 该模式只影响短视频，不影响长视频。
4. 该模式用于验证平台行为，不建议用于正式发布。
...
- 短视频背景音乐默认“仅授权音轨池自动选用”；未提供授权音轨时自动降级为纯人声，支持通过测试开关临时验证平台行为。
